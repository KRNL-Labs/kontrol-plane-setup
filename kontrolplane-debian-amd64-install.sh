...
# S3 public URLs for binary & config (HTTP GET accessible)
...
Run this script as root on Debian 11/12.
USAGE
      exit 0 ;;
...
ensure_service_user_and_dirs() {
  ...
  # Add to docker group after Docker is installed (group will exist)
...
echo "[Packages] base"
apt-get update -y
apt-get install -y ca-certificates curl wget gnupg apt-transport-https lsb-release

# ---------------- Docker (official APT repo for Debian) ----------------
